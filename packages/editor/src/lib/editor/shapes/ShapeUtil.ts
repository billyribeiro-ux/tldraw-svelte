...
import type { LegacyMigrations, MigrationSequence } from '@tldraw/store'
import type {
	RecordProps,
	TLAsset,
	TLHandle,
...
import type { TLFontFace } from '@tldraw/tlschema'
import type { IndexKey } from '@tldraw/utils'
import type { Box, SelectionHandle } from '../../primitives/Box'
import type { Geometry2d } from '../../primitives/geometry/Geometry2d'
import type { Vec } from '../../primitives/Vec'
import type { Editor } from '../Editor'
import type { BoundsSnapGeometry } from '../managers/SnapManager/BoundsSnaps'
import type { HandleSnapGeometry } from '../managers/SnapManager/HandleSnaps'
import type { TLClickEventInfo } from '../types/event-types'
import type { TLResizeHandle } from '../types/selection-types'
import type { SvgExportContext } from '../types/SvgExportContext'
...
/**
 * A canvas-level SVG `<defs>` entry contributed by a shape util (e.g. a pattern
 * or filter). In the Svelte port `component` is framework-agnostic (rendered by
 * a Svelte component in the canvas layer) rather than a React `ComponentType`.
 *
 * @public
 */
...
export abstract class ShapeUtil<Shape extends TLShape = TLShape> {
	 ...
	 * @public
	 */
	toSvg?(shape: Shape, ctx: SvgExportContext): unknown | null | Promise<unknown | null>
