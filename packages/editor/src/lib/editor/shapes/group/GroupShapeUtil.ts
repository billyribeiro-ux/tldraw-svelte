import type { TLGroupShape} from '@tldraw/tlschema';
...
export class GroupShapeUtil extends ShapeUtil<TLGroupShape> {
	...
	// NOTE (Svelte port): the React `component()` returned
	// `<SVGContainer><DashedOutlineBox .../></SVGContainer>` to draw the dashed
	// group outline. The decision logic (when the outline is shown) is preserved
	// below; the Svelte renderer (Phase 2/3) maps the returned descriptor to a
	// <GroupShape.svelte>. For Phase 1 (no group tool) this returns null.
	component(shape: TLGroupShape) {
		...
		// Outline should be shown; bounds = this.editor.getShapeGeometry(shape).bounds
		// Rendering deferred to the Svelte layer.
		return null
	}
