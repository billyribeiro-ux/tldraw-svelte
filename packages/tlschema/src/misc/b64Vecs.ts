...
import type { VecModel } from './geometry-types'
...
// NOTE (Svelte port): the original declared `Uint8Array.toBase64?`/
// `Uint8ArrayConstructor.fromBase64?` as optional global augmentations. TS 6.0's
// lib.esnext now declares these as REQUIRED, so the local optional augmentation
// conflicted ("overload signatures must all be optional or required"). Removed —
// the runtime feature-detection below still guards older environments.
