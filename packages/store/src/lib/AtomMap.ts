...
import type { ImmutableMap } from './ImmutableMap';
import { emptyMap } from './ImmutableMap'
...
export class AtomMap<K, V> implements Map<K, V> {
	...
	// NOTE (Svelte port): TS 6.0's lib added the TC39 "upsert" methods
	// `getOrInsert`/`getOrInsertComputed` to the `Map` interface. `AtomMap`
	// `implements Map`, so it must provide them. Implemented in terms of the
	// existing reactive `has`/`get`/`set`.

	/** Returns the value for `key`, inserting `defaultValue` first if absent. */
	...
	/** Returns the value for `key`, inserting the result of `callbackfn` first if absent. */
	...
	*values(): Generator<V, undefined, unknown> {
	 ...
	 * ```
	 */
	// NOTE (Svelte port): the callback's `map` param is typed `Map<K, V>` (not
	// `AtomMap<K, V>`) to stay assignable to the `Map.forEach` base signature
	// under TS 6.0's lib, which added `getOrInsert`/`getOrInsertComputed` to Map.
