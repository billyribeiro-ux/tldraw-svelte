...
// Svelte port: the original React type imports `ReactNode, SVGProps from 'react'`
// were types-only, used by the (now-stubbed) `toSvg()` JSX path. Replaced with
// local framework-free equivalents so the geometry/D-string logic compiles
// without React. The Svelte renderer uses toD()/toDrawD()/toPath2D() instead.
type ReactNode = unknown
type SVGProps<_T> = Record<string, unknown>
...
export class PathBuilder {
	...
	toSvg(_opts: PathBuilderOpts): ReactNode {
		throw new Error(
			'PathBuilder.toSvg is not used in the Svelte port; render via toD()/toDrawD() in the Svelte component'
		)
	}
	...
	private toSolidSvg(_opts: PathBuilderOpts): ReactNode {
		// Svelte port: original returned React <path> JSX; only reachable via the
		// stubbed toSvg(). Render via toD()/toPath2D() in the Svelte component instead.
		throw new Error('PathBuilder.toSolidSvg is not used in the Svelte port; use toD()/toPath2D()')
	}

	private toDashedSvg(opts: DashedPathBuilderOpts): ReactNode {
		// Svelte port: original returned React <g>/<path> JSX; only reachable via the
		// stubbed toSvg(). The dash-run/length computation below is preserved so the
		// Svelte renderer can reuse the per-run dash props logic, but the JSX emission
		// is replaced with a throw. Render via toD() + getPerfectDashProps() instead.
		const {
			style,
			strokeWidth,
			snap,
		...
		const addCurrentRun = () => {
			...
			getPerfectDashProps(length, strokeWidth, {
				style,
				snap,
				lengthRatio,
	...
	private toDrawSvg(_opts: DrawPathBuilderOpts): ReactNode {
		...
		throw new Error('PathBuilder.toDrawSvg is not used in the Svelte port; use toDrawD()')
	}
