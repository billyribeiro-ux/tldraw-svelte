...
// guarded: highlight not yet converted — the React HighlightShapeUtil is not vendored
// in the Svelte port. The draw tool only ever creates/edits `draw` shapes here, so the
// HighlightShapeUtil value import is dropped and its type is replaced with `any` below.
import { STROKE_SIZES } from '../../shared/default-shape-constants'
import { getDisplayValues } from '../../shared/getDisplayValues'
import type { DrawShapeUtil } from '../DrawShapeUtil'
...
export class Drawing extends StateNode {
	...
	// guarded: highlight not yet converted — was `DrawShapeUtil | HighlightShapeUtil`.
	util = this.editor.getShapeUtil(this.shapeType) as DrawShapeUtil | any
