...
import type { StrokeOptions } from '../shared/freehand/types'
...
// Svelte port: `getDot` is a local helper in the React `DrawShapeUtil.tsx` /
// `DrawShapeSvg` component. It is moved here (framework-free, identical body) so the
// Svelte `DrawShape.svelte` renderer can import it as a single source of truth.
/** @public */
export function getDot(point: VecLike, sw: number) {
	const r = (sw + 1) * 0.5
