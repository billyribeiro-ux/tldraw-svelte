...
import type {
	...
	TLLineShape,
	TLLineShapePoint,
	TLResizeInfo} from '@tldraw/editor';
import {
	Group2d,
	ShapeUtil,
	Vec,
...
export class LineShapeUtil extends ShapeUtil<TLLineShape> {
	...
	// Svelte port: rendering handled by LineShape.svelte (registry by type), which
	// uses the exported getPathForLineShape() + display values. component() unused.
	component() {
		return null
	}
	...
	override toSvg(_shape: TLLineShape, _ctx: SvgExportContext) {
		// SVG export ported in Phase G.
		throw new Error('LineShapeUtil.toSvg not yet ported to Svelte (Phase G).')
	}
...
export function linePointsToArray(shape: TLLineShape) {
	return Object.values(shape.props.points).sort(sortByIndex)
}

const pathCache = new WeakCache<TLLineShape, PathBuilder>()
export function getPathForLineShape(shape: TLLineShape): PathBuilder {
...
// NOTE (Svelte port): the React `LineShapeSvg` renderer was removed; LineShape.svelte
// reproduces it using the exported getPathForLineShape() + toD()/toDrawD().
