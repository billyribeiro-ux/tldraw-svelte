...
import { type ArrowShapeOptions, type ArrowShapeUtilDisplayValues } from './arrow-types'
import { getArrowheadPathForType } from './arrowheads'
import { getArrowLabelDefaultPosition, getArrowLabelPosition } from './arrowLabel'
import { getArrowBodyPathBuilder } from './ArrowPath'
import { updateArrowTargetState } from './arrowTargetState'
import { ElbowArrowAxes } from './elbow/definitions'
// debug overlay omitted in Svelte port: the React `./elbow/ElbowArrowDebug`
// overlay is dev-only and not vendored.
...
export class ArrowShapeUtil extends ShapeUtil<TLArrowShape> {
	...
	override getFontFaces(shape: TLArrowShape) {
		...
		// Svelte port: the original first looked up theme font faces via
		// `getThemeFontFaces` from `../shared/defaultFonts` (a React render helper
		// we do not vendor). Fall back to deriving faces from the rich text only,
		// matching the vendored GeoShapeUtil.
	...
	component(_shape: TLArrowShape) {
		// Svelte port: rendering is handled by the Svelte component (ArrowShape.svelte),
		// not by a React component() method.
		//
		// The original component() returned an <SVGContainer> wrapping an <ArrowSvg>
		// (which drew the clipped arrow body via getArrowBodyPath() plus start/end
		// arrowhead <path>s via getArrowheadPathForType(), optionally pattern-filled),
		// an optional <ElbowArrowDebug> overlay, and an optional <RichTextLabel>
		// positioned via getArrowLabelPosition(). The Svelte renderer reproduces this
		// using the exported helpers (getArrowInfo, getArrowBodyPathBuilder,
		// getArrowheadPathForType, getArrowLabelPosition).
		return null
	}
	...
	override toSvg(_shape: TLArrowShape, _ctx: SvgExportContext) {
		// Svelte port: the original returned React JSX (<g> wrapping <ArrowSvg> +
		// optional <RichTextSVG>). SVG export is handled by the Svelte renderer.
	...
	override getCanvasSvgDefs(): TLShapeUtilCanvasSvgDef[] {
		...
		// render helper `../shared/defaultStyleDefs` (not vendored). The Svelte
		// renderer supplies its own canvas SVG defs.
		return []
	}
