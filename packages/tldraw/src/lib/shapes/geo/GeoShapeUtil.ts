...
export class GeoShapeUtil extends BaseBoxShapeUtil<TLGeoShape> {
	...
	override getFontFaces(shape: TLGeoShape) {
		...
		// Svelte port: the original looked up theme font faces via
	...
	component(_shape: TLGeoShape) {
		// Svelte port: rendering is handled by the Svelte component, not by a
		// React component() method. See the renderer in the Svelte app.
		return null
	}
	...
	override toSvg(_shape: TLGeoShape, _ctx: SvgExportContext) {
		// Svelte port: the original returned React JSX (GeoShapeBody + RichTextSVG).
		// SVG export is handled by the Svelte renderer instead.
		throw new Error(
			'GeoShapeUtil.toSvg is not used in the Svelte port; SVG export is handled by the Svelte renderer'
		)
	}
