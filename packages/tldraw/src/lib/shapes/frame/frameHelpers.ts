import type {
	...
	TLFrameShape} from '@tldraw/editor';
import {
	Box,
...
/**
 * Svelte port: the real `frameHelpers.ts` imports `TLCreateTextJsxFromSpansOpts`
 * from the React render helper `../shared/createTextJsxFromSpans` (a `.tsx` file
 * we deliberately do NOT vendor). That type is itself just `TLMeasureTextSpanOpts`
 * (the structural options accepted by `editor.textMeasure.measureTextSpans`) plus a
 * few SVG fields. `TLMeasureTextSpanOpts` is not re-exported from the Svelte port's
 * `@tldraw/editor` barrel, so we mirror the structural shape locally here. This keeps
 * `getFrameHeadingOpts`/`getFrameHeadingSize` framework-free while still producing an
 * object that `measureTextSpans` accepts structurally.
 */
export interface TLCreateTextJsxFromSpansOpts {
