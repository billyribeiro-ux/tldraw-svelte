import type {
	...
	TLFrameShapeProps,
	TLShapePartial,
	TLShapeUtilConstructor} from '@tldraw/editor';
import {
...
export class FrameShapeUtil extends BaseFrameLikeShapeUtil<TLFrameShape> {
	...
	override component(_shape: TLFrameShape) {
		// Svelte port: rendering is handled by the Svelte `<FrameShape>.svelte`
		// component, not by a React `component()` method. The original returned a
		// `<SVGContainer>` with a `tl-frame__body` <rect> plus a `<FrameHeading>`
		// React sub-component (neither vendored here). See the Svelte renderer, which
		// reads the display values via `getDisplayValues(util, shape, colorMode)`.
		return null
	}

	override toSvg(_shape: TLFrameShape, _ctx: SvgExportContext) {
		// Svelte port: the original returned React JSX (a <rect> + a heading <g>
		// built with `createTextJsxFromSpans`, a React render helper we do not
		// vendor). SVG export is handled separately in a later phase.
