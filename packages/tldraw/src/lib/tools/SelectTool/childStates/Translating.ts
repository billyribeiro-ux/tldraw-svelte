import type {
	...
	TLShape,
	TLShapePartial,
	TLTickEventInfo} from '@tldraw/editor';
import {
	Mat,
	PageRecordType,
	StateNode,
...
// guarded: note not yet converted — noteHelpers/NoteShapeUtil live in shapes/note/*.
// The note snapping path is only reached when a 'note' shape is hovered (see allHoveredNotes
// below, filtered by isShapeOfType(..., 'note')), which is never true for geo, so these
// stand-ins are unreachable at runtime for converted shapes.
const NOTE_ADJACENT_POSITION_SNAP_RADIUS = 10
const getAvailableNoteAdjacentPositions = (_editor: Editor, _opts: any): Vec[] => []
...
function getTranslatingSnapshot(editor: Editor) {
	...
	// guarded: note not yet converted — only run the note-adjacent-position logic when the
	// 'note' shape util is registered. For geo-only setups noteSnapshot is already undefined.
	if (noteSnapshot && editor.getShapeUtil('note')) {
		const noteUtil = editor.getShapeUtil(noteSnapshot.shape) as NoteShapeUtil
		// guarded: note not yet converted — display values carry note dims when the real util exists.
		const dv = getDisplayValues(noteUtil, noteSnapshot.shape) as {
			noteWidth: number
			noteHeight: number
		}
