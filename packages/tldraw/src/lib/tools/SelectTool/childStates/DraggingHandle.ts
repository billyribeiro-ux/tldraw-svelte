import type {
	...
	TLPointerEventInfo,
	TLShapeId,
	TLShapePartial} from '@tldraw/editor';
import {
	StateNode,
	Vec,
...
// guarded: arrow not yet converted — ArrowShapeUtil/arrowTargetState/shared live in shapes/arrow/*.
// ArrowShapeUtil type replaced with `any` (getShapeUtil<T> constrains T to ShapeUtil<TLShape>,
// which a minimal local shape can't satisfy). Only reached on arrow shapes, never geo.
type ArrowShapeUtil = any
// clearArrowTargetState is called unconditionally in onExit. It only clears transient arrow-target
// hinting state; with no arrow shape present there's nothing to clear, so this is a safe no-op.
const clearArrowTargetState = (_editor: Editor): void => {}
// getArrowBindings is only reached behind editor.isShapeOfType(shape, 'arrow') guards below,
// never true for geo/line — stand-in returns an empty binding map.
const getArrowBindings = (
	_editor: Editor,
	_shape: any
): Record<'start' | 'end', { toId: TLShapeId; props: { isPrecise: boolean } } | undefined> => ({
	start: undefined,
	end: undefined,
})
...
export class DraggingHandle extends StateNode {
	...
	private resetExactTimeout() {
		const arrowUtil = this.editor.getShapeUtil<ArrowShapeUtil>('arrow') as any
