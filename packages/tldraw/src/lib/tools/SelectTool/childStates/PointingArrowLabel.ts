import type {
	...
	TLArrowShape,
	TLPointerEventInfo,
	TLShapeId} from '@tldraw/editor';
import {
	StateNode,
	Vec,
} from '@tldraw/editor'
// guarded: arrow not yet converted — this whole state is arrow-only. It is only entered via the
// 'pointing_arrow_label' transition, which is gated behind isOverArrowLabel (a no-op for geo),
// so none of these stand-ins run at runtime for converted shapes.
const getArrowBodyGeometry = (_editor: Editor, _shape: any): any => undefined
const getArrowLabelDefaultPosition = (_editor: Editor, _shape: any): number => 0.5
...
export class PointingArrowLabel extends StateNode {
	...
	override onPointerMove() {
		...
		const options = (this.editor.getShapeUtil<ArrowShapeUtil>('arrow') as any).options
